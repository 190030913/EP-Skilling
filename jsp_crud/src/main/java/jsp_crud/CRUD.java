...
public class CRUD {
	public int Insert(Bean eb) throws ClassNotFoundException,SQLException{
		...
		String sql="insert into jsp_crud values(?,?,?)";
	...
	public  int updation(Bean eb) throws ClassNotFoundException, SQLException {
		Connection con=DBUtil.DBConnection();		
		PreparedStatement stmt=con.prepareStatement("update jsp_crud set name=?,email=? where id=?");  
	...
	public  int Delete(int id) throws ClassNotFoundException, SQLException {
		Connection con=DBUtil.DBConnection();		
		PreparedStatement stmt=con.prepareStatement("delete from jsp_crud where id=?");  
		stmt.setInt(1,id);
		int u=stmt.executeUpdate();  
		con.close();
		return u;
	...
	public static Bean getRecordById(int id){  
	    ...
	    try{  
	    	 Connection con=DBUtil.DBConnection();   
	        PreparedStatement ps=con.prepareStatement("select * from jsp_crud where id=?");  
	        ps.setInt(1,id);  
